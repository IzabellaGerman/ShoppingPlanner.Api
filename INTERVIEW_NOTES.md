...
### Почему разные `DeleteBehavior` для двух FK в `ShoppingListItem`

`ShoppingListItem` имеет две FK: `ShoppingListId` (Cascade) и `ProductId` (Restrict).

**Почему разное поведение:**
- `ShoppingList → Items`: жизненный цикл item привязан к списку. Удалили список — удалились его items. `Cascade`.
- `Product`: справочная сущность, существует независимо от того, в каких списках упоминается. Удаление продукта не должно "тихо" удалять историю списков. `Restrict` — EF не даст удалить продукт, если на него есть ссылки.

**Follow-up:**
- "У тебя обе FK в одной таблице, почему cascade поведение разное?" → Потому что это две разные связи по смыслу: одна — "часть-целое" (item принадлежит списку), другая — "ссылка на справочник" (item ссылается на продукт). Cascade подходит для первой, Restrict — для второй, чтобы не терять данные случайно.
